...
class TransformerAgent(Agent):
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared is None:
            ...
            # self.model.load_state_dict(state_dict, restrict='False')
    ...
    def _parse_v1(self, text):
        ...
        for subtext in text.split('\n'):
            ...
            if subtext.startswith('your persona:'):
                subtext = subtext.replace('your persona:', '').strip()
                subtext = self._preprocess_text(subtext).strip()
                persona_info.append([self.vocab.info_bos_id] + self.vocab.string2ids(subtext) + [self.vocab.info_eos_id])
                persona_len = [len(i)-2 for i in persona_info]
            else:
                subtext = self._preprocess_text(subtext).strip()
                dialog.append(subtext)

        return persona_info, persona_len, dialog
    ...
    def observe(self, observation):
        ...
        if 'text' in observation:
            ...
            info, persona_len, dialog = self._parse_v1(text)

            # info = sum([self.vocab.string2ids(i) for i in info], [])
            info = sum(info, [])
            self.history['info'].extend(info)
            self.history['info_len'].extend(persona_len)
    ...
    def batch_act(self, observations):
        ...
        try:
            ...
            infos_len = [obs['agent'].history['info_len'] for obs in valid_observations]
            contexts = []
            ...
            context = self.model.encode(dialogs)
            persona, out_weights = self.model.weighted_encode(dialogs, infos, infos_len)
    ...
    def reset(self):
        self.history = {'str_info': None, 'str_dialog': deque(DIALOG_SIZE * ['None'], maxlen=DIALOG_SIZE),
                        'info': [], 'info_len': [], 'dialog': deque(maxlen=self.model.n_pos_embeddings-1)}
