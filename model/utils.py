...
def pad_sequence_of_sequence(sequences, batch_first=False, padding_value=0, max_len=0, padding_sentence_value=0):
    # assuming trailing dimensions and type of all the Tensors
    # in sequences are same and fetching those from sequences[0]
    max_size = sequences[0][0].size()
    trailing_dims = max_size[1:]
    max_len = max(max([s.size(0) for seq in sequences for s in seq]), max_len)
    max_group = max([len(s) for s in sequences])
    if batch_first:
        out_dims = (len(sequences), max_group, max_len) + trailing_dims
    else:
        out_dims = (max_group, max_len, len(sequences)) + trailing_dims

    out_tensor = sequences[0][0].data.new(*out_dims).fill_(padding_value)
    for j, group in enumerate(sequences):
        for i in range(max_group): # enumerate(group):
            try:
                length = group[i].size(0)
                # use index notation to prevent duplicate references to the tensor
                if batch_first:
                    out_tensor[j, i, :length, ...] = group[i]
                else:
                    out_tensor[:length, i, j, ...] = group[i]
            except: # set </end_of_info> for padded sentence
                if batch_first:
                    out_tensor[j, i, 0, ...] = padding_sentence_value
                else:
                    out_tensor[0, i, j, ...] = padding_sentence_value
