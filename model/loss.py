...
class LabelSmoothingLoss(nn.Module):
    def __init__(self, n_labels, smoothing=0.0, ignore_index=-100, size_average=True):
        ...
        if smoothing > 0:
            self.criterion = nn.KLDivLoss(reduction='mean' if size_average else 'sum')
            ...
            self.criterion = nn.NLLLoss(reduction='mean' if size_average else 'sum', ignore_index=ignore_index)
